...
public class ModifyServlet extends AbstractServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try (Connection connection = getConnection(req.getServletContext())) {
            ...
            for(String email : emails){
                userDao.deleteFromAttendance(email, (String) req.getSession().getAttribute("date"));
            }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try (Connection connection = getConnection(req.getServletContext())) {
            ...
            req.setAttribute("users", userDao.listAttendance(req.getParameter("date")));
            req.getSession().setAttribute("date",req.getParameter("date"));
